...
const nombreRegex = /^[A-Za-z\s]{1,}$/;
const precioRegex = /^\d+(\.\d{1,2})?$/;
const urlRegex = /^(ftp|http|https):\/\/[\w\-]+(\.[\w\-]+)+([\w\-\.,@?^=%&:\/~\+#]*[\w\-\@?^=%&\/~\+#])?$/;
const descripcionRegex = /^[\w\s]{1,}$/;
...
function modal() {
    ...
    function validarFormulario() {
      ...
      if (!esNombreValido) {
        alert("El Nombre es Invalido,recuerda al menos 1 mayusculas");
      ...
      if (!esPrecioValido) {
        alert("El Precio es Invalido , recuerda solo valores numericos");
      ...
      if (!esURLValida) {
        alert("El URL es Invalido,recuerda comenzar con ftp o http o https");
      ...
      if (!esDescripcionValida) {
        alert("El Descripcion es Invalido");
